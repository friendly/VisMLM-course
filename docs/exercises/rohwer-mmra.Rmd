...
```{r loadpkgs, class.source = 'fold-show'}
library(car)
library(heplots)
library(stargazer)
```
...
```{r read-rohwer, class.source = 'fold-show'}
...
```{r unimods, class.source = 'fold-show'}
...
```{r rohwer-mlm, class.source = 'fold-hide'}
...
```{r lh-mlm, class.source = 'fold-hide'}
predictors <- c("n", "s", "ns", "na", "ss")
...
```{r he-pairs, class.source = 'fold-show'}
pairs(rohwer.mlm, 
      hypotheses=hyp, 
      fill = TRUE,
      col = cols)
